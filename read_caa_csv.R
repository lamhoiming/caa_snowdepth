...
#################### Switch #################
#############################################
do.monthtrend <- TRUE #FALSE           #######
do.monthts <- FALSE #TRUE             #######
...
for (x in 1:length(list_stn)) {
  ...
  if (do.monthtrend == TRUE) {
      ...
      for (mth in 1:12) {  #monthly trend
        if (any(cycle(stn_mean_byyearmonth) == mth)){ #test for if data exist in that month
          ...
          if (length(dfmthmean) < 7) { # At least 7 years of data to plot trend
            ...
            write.csv(dfmth, file = paste(stn_title, '_', rnames[mth], '.csv', sep =''), 
                      row.names = format(index(dfmth), "%Y"), na = "NA")
            
            trd <- read.csv(file = paste(stn_title, '_', rnames[mth], '.csv', sep =''), 
            ...
            png(paste(stn_title, '_', rnames[mth], '_month_trend.png', sep = ''), 
                width = 800, height = 567, units="px")
            
            errbar(trd$yr, trd$mean, trd$mean + trd$sd, trd$mean - trd$sd, 
                   ylim = c(0,100), las = 1, type = "o", xlab = "Year", ylab = "Snow depth (cm)")
    ...
    svdir <- paste(Mdir, "output/graphs/trend/monthly/stats", sep = "")
    setwd(svdir)
    ...
    write.csv(stats.out, file = paste(stn_title, '_month_trend_stats.csv', sep = ''))
    
    # plot trend by month with SONDJFMAMJJ
    svdir <- paste(Mdir, "output/graphs/trend/monthly/stats/plots", sep = "")
    setwd(svdir)
    pdf(paste(stn_title, '_trend_by_month.pdf', sep = ''))
    png(paste(stn_title, '_trend_by_month.png', sep = ''), 
